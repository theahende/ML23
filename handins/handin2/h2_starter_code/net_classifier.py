...
class NetClassifier:
    ...
    def cost_grad(X, y, params, c=0.0):
        ...
        # To avoid numerical problems from taking the log of 0
        sm_z_correct = np.clip(sm_z_correct, 1e-15, 1 - 1e-15)
        weight_decay = c * (np.sum(W1**2) + np.sum(W2**2))
        cost = (np.mean(-np.log(sm_z_correct))) + weight_decay

        # Backwards pass
        d_sm_z = sm_z - labels
        d_b2 = np.mean(d_sm_z, axis=0, keepdims=True)
        d_w2 = ((our_c.T @ d_sm_z) / len(y)) + c * 2 * W2

        d_sm_z_2 = d_sm_z @ W2.T
        d_relu = derive_relu(d_sm_z_2, d)
        d_b1 = np.mean(d_relu, axis=0, keepdims=True)
        d_w1 = ((X.T @ d_relu) / len(y)) + (2 * c * W1)
        # ### END CODE
    ...
    def fit(
        ...
        for _ in range(epochs):
            ...
            for j in range(numOfBatches):
                ...
                eta = lr
                cost, grad = self.cost_grad(X_batch, Y_batch, params, c)
                W1 = W1 - eta * grad["d_w1"]
                W2 = W2 - eta * grad["d_w2"]
                b1 = b1 - eta * grad["d_b1"]
                b2 = b2 - eta * grad["d_b2"]
            ...
            val_loss.append(self.cost_grad(X_val, y_val, params)[0])
